    ...
    for e in range(epochs):
        ...
        for batch in progressbar(trainloader):
            ...
            if steps % valid_every == 0:
                ...
                with open(
                    os.path.join(checkpoints_dir, "history.json"), mode="w"
                ) as fp:
                ...
                ax1.legend()
                ax2.legend()
                fig.savefig(os.path.join(checkpoints_dir, "history.jpg"))
                plt.close(fig)
...
def _plot_history(history, ax_loss, ax_accuracy, label_suffix="", index=-1):
    ...
    valid_steps = (
        (np.array(range(len(valid_loss_history))) + 1) * valid_every / epoch_size
    )
    ...
    if index >= 0:
        train_style = {
            "color": list(TABLEAU_COLORS)[index],
        }
        valid_style = {"color": list(TABLEAU_COLORS)[index], "ls": "--"}
    ax_loss.plot(
        train_steps,
        train_loss_history,
        label="training loss" + label_suffix,
        **train_style,
    )
    ax_loss.plot(
        valid_steps,
        valid_loss_history,
        label="validation loss" + label_suffix,
        **valid_style,
    )
    ax_accuracy.plot(
        train_steps,
        train_accuracy_history,
        label="training accuracy" + label_suffix,
        **train_style,
    )
    ax_accuracy.plot(
        valid_steps,
